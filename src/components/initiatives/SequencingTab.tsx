...
export default function SequencingTab({
  initiativeId,
}: {
  initiativeId: string;
}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setMaxMonth(dep + 6);
      setSequencingNotes(
        ((lbc as any)?.sequencing_dependencies as string) ?? ""
      );
      setLoading(false);
    })();
  ...
  const addRow = () =>
    setRows((prev) => [...prev, { id: null, feature_id: null, month_start: 1 }]);

  const deleteRow = (idx: number) =>
    setRows((prev) => {
      const r = prev[idx];
      if (!r.id) return prev.filter((_, i) => i !== idx);
  ...
  const usedFeatureIds = useMemo(() => {
    ...
    rows.forEach((r) => {
      if (r._deleted || !r.feature_id) return;
  ...
  const handleSave = async () => {
    ...
    for (const r of rows) {
      ...
      if (!r.feature_id) { toast.error("Every row must have a feature selected."); return; }
      if (!r.month_start || r.month_start < 1) { toast.error("Every row must have a start month >= 1."); return; }
    }
    for (const [, count] of usedFeatureIds) {
      if (count > 1) { toast.error("Duplicate feature in sequence."); return; }
    }

    setSaving(true);
    try {
      const activeRows = rows.filter((r) => !r._deleted && r.feature_id);
      const keepFeatureIds = activeRows.map((r) => r.feature_id as string);

      // Step 1 — delete removed rows from DB
      if (keepFeatureIds.length > 0) {
        const { error: delErr } = await supabase
          ...
          .eq("initiative_id", initiativeId)
          .eq("item_type", "feature")
          .not("feature_id", "in", `(${keepFeatureIds.join(",")})`);
      ...
      // Step 2 — upsert with always-present id (KEY FIX)
      const toUpsert = activeRows.map((r) => ({
      ...
      console.error("Sequencing save error:", e);
  ...
  return (
    <div className="space-y-6">
              ...
              {visibleRows.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-4 text-center text-muted-foreground">
                    No sequencing items yet. Click "+ Add Feature" to begin.
                  </td>
                </tr>
              )}
              {visibleRows.map(({ r, i }) => {
                ...
                return (
                  <tr key={r.id ?? `new-${i}`} className="border-t border-slate-100 align-top">
                    ...
                    <td className="px-2 py-2">
                      {f ? (
                        f.is_mvp
                          ? <Badge variant="outline" className="border-blue-300 text-blue-700 bg-blue-50">MVP</Badge>
                          : <Badge variant="outline" className="border-slate-300 text-slate-600 bg-slate-50">Post-MVP</Badge>
                    ...
                    <td className="px-2 py-2">
                      <Input
                        ...
                        max={maxMonth}
                        value={r.month_start}
                        onChange={(e) => updateRow(i, { month_start: parseInt(e.target.value, 10) || 1 })}
                    ...
                    <td className="px-2 py-2 text-right">
                      {canEdit && (
                        <button type="button" onClick={() => deleteRow(i)}
                          className="text-slate-400 hover:text-red-600" aria-label="Delete row">
      ...
      <div className="space-y-2">
        <div className="text-sm font-semibold text-slate-700">Sequencing Notes (LBC narrative)</div>
        <Textarea value={sequencingNotes} readOnly rows={5} className="text-xs bg-slate-50"
          placeholder="(No sequencing narrative recorded in the LBC.)" />
        <p className="text-[11px] text-muted-foreground">
          This is the narrative from the LBC document. The structured timeline above is the live sequencing record.
        </p>
      </div>
    </div>
  );
