      ...
      if (metrics && metrics.length > 0) {
        const outcomes = metrics
          ...
          .map((m: any) => ({
            ...
            metric_direction: (m.metric_direction === "accumulation" ? "accumulation" : "reduction") as "reduction" | "accumulation",
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={e => {
                                const raw = e.target.value;
                                const v = raw === "" ? null : Math.trunc(Number(raw));
                                setPostMvpRows(prev => prev.map((r, i) => i === idx ? { ...r, duration_months: v } : r));
                              }}
                              placeholder="1–3"
                              className="w-24"
                              {...fieldProps()}
